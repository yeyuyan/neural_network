...
def create_model():
    model = Sequential()
    model.add(LSTM(units = 4, batch_input_shape = (batch_size, look_back, 1), 
                   stateful = True))
...
if __name__ == '__main__':
    ...
    for i in range(epochs):
        history = model.fit(X_train, y_train, epochs = epochs, batch_size = 
                            batch_size, verbose = 0, shuffle = False)
    ...
    predict_validation_plot[len(predict_train)+look_back*2 :len(dataset),:] = predict_validation
